...
void adc1_init(void) {
	...
	/* DMA */
	ADC1->CFGR |= ADC_CFGR_DMAEN | ADC_CCR_DMACFG; // DMA enabled; Circular mode
...
void dma1_init(void) {
	...
	DMA1_Channel1->CCR |= DMA_CCR_MSIZE_0 | DMA_CCR_PSIZE_0; // 16-bits peripheral & memory size
	...
	DMA1_Channel1->CCR |= DMA_CCR_HTIE; // Half-transfer complete interrupt enabled
	DMA1_Channel1->CCR |= DMA_CCR_TCIE; // Transfer complete interrupt enabled
	DMA1_Channel1->CCR |= DMA_CCR_TEIE; // Transfer error interrupt enabled
